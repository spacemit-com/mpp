...
#define TEST_ASSERT(cond)                                               \
    do {                                                                \
        if (!(cond))                                                    \
            UnityFail("Expected TRUE Was FALSE", (UNITY_UINT)__LINE__); \
    } while (0)
#define TEST_ASSERT_TRUE(cond) TEST_ASSERT(cond)
#define TEST_ASSERT_FALSE(cond)                                         \
    do {                                                                \
        if (cond)                                                       \
            UnityFail("Expected FALSE Was TRUE", (UNITY_UINT)__LINE__); \
    } while (0)
#define TEST_ASSERT_NULL(ptr)                                 \
    do {                                                      \
        if ((ptr) != NULL)                                    \
            UnityFail("Expected NULL", (UNITY_UINT)__LINE__); \
    } while (0)
#define TEST_ASSERT_NOT_NULL(ptr)                                 \
    do {                                                          \
        if ((ptr) == NULL)                                        \
            UnityFail("Expected non-NULL", (UNITY_UINT)__LINE__); \
    } while (0)
...
#define TEST_ASSERT_EQUAL_INT(expected, actual)                         \
    do {                                                                \
        if (static_cast<int>(expected) != static_cast<int>(actual)) {                         \
            ...
            UnityFail("Expected equal integers", (UNITY_UINT)__LINE__); \
        }                                                               \
    } while (0)
#define TEST_ASSERT_NOT_EQUAL(expected, actual)                           \
    do {                                                                  \
        if ((expected) == (actual))                                       \
            UnityFail("Expected different values", (UNITY_UINT)__LINE__); \
    } while (0)
...
#define TEST_ASSERT_EQUAL_UINT(expected, actual)                                 \
    do {                                                                         \
        if ((unsigned)(expected) != (unsigned)(actual))                          \
            UnityFail("Expected equal unsigned integers", (UNITY_UINT)__LINE__); \
    } while (0)
...
#define TEST_ASSERT_EQUAL_PTR(expected, actual)                         \
    do {                                                                \
        if ((expected) != (actual))                                     \
            UnityFail("Expected equal pointers", (UNITY_UINT)__LINE__); \
    } while (0)
...
#define TEST_ASSERT_GREATER_THAN(threshold, actual)                             \
    do {                                                                        \
        if (!((actual) > (threshold)))                                          \
            UnityFail("Expected greater than threshold", (UNITY_UINT)__LINE__); \
    } while (0)
#define TEST_ASSERT_LESS_THAN(threshold, actual)                             \
    do {                                                                     \
        if (!((actual) < (threshold)))                                       \
            UnityFail("Expected less than threshold", (UNITY_UINT)__LINE__); \
    } while (0)
#define TEST_ASSERT_GREATER_OR_EQUAL(threshold, actual)                                \
    do {                                                                               \
        if (!((actual) >= (threshold)))                                                \
            UnityFail("Expected greater or equal to threshold", (UNITY_UINT)__LINE__); \
    } while (0)
#define TEST_ASSERT_LESS_OR_EQUAL(threshold, actual)                                \
    do {                                                                            \
        if (!((actual) <= (threshold)))                                             \
            UnityFail("Expected less or equal to threshold", (UNITY_UINT)__LINE__); \
    } while (0)

/* --- String --- */
#define TEST_ASSERT_EQUAL_STRING(expected, actual)                                                       \
    do {                                                                                                 \
        if (__extension__ __builtin_expect(                                                              \
                (expected) == NULL || (actual) == NULL || __builtin_strcmp((expected), (actual)) != 0, 0)) \
            UnityFail("Expected equal strings", (UNITY_UINT)__LINE__);                                   \
    } while (0)
