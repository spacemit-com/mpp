...
S32 venc_ctx_init(MppVencCtx *ctx) {
    ...
    enc_init = (void (*)(ALBaseContext *ctx, MppVencPara *para))dlsym(handle, "al_enc_init");
    enc_set_para = (S32 (*)(ALBaseContext *ctx, MppVencCmd cmd, void *para))dlsym(handle, "al_enc_set_para");
    enc_return_input_frame =
        (S32 (*)(ALBaseContext *ctx, MppData *sink_Data))dlsym(handle, "al_enc_return_input_frame");
    enc_send_input_frame = (S32 (*)(ALBaseContext *ctx, MppData *sink_Data))dlsym(handle, "al_enc_send_input_frame");
    enc_encode = (S32 (*)(ALBaseContext *ctx, MppData *sink_data))dlsym(handle, "al_enc_encode");
    enc_process = (S32 (*)(ALBaseContext *ctx, MppData *sink_data, MppData *src_data))dlsym(handle, "al_enc_process");
    enc_get_output_stream = (S32 (*)(ALBaseContext *ctx, MppData *src_Data))dlsym(handle, "al_enc_get_output_stream");
    enc_request_output_stream =
        (S32 (*)(ALBaseContext *ctx, MppData *src_Data))dlsym(handle, "al_enc_request_output_stream");
    enc_return_output_stream =
        (S32 (*)(ALBaseContext *ctx, MppData *src_Data))dlsym(handle, "al_enc_return_output_stream");
    enc_flush = (S32 (*)(ALBaseContext *ctx))dlsym(handle, "al_enc_flush");
    enc_destory = (void (*)(ALBaseContext *ctx))dlsym(handle, "al_enc_destory");

    if (!enc_create || !enc_init || !enc_encode || !enc_destory) {
        error(
            "required encoder symbols missing from plugin "
    ...
    enc_init(ctx->pAlBaseContext, &(ctx->stVencPara));
...
S32 venc_ctx_set_param(MppVencCtx *ctx, MppVencCmd cmd, void *para) {
    S32 ret = enc_set_para(ctx->pAlBaseContext, cmd, para);
    return ret;
}
...
S32 venc_ctx_send_input_frame(MppVencCtx *ctx, MppData *sink_data) {
    S32 ret = 0;
    ret = enc_send_input_frame(ctx->pAlBaseContext, sink_data);
    return ret;
}

S32 venc_ctx_return_input_frame(MppVencCtx *ctx, MppData *sink_data) {
    S32 ret = 0;
    ret = enc_return_input_frame(ctx->pAlBaseContext, sink_data);
    return ret;
}
...
S32 venc_ctx_encode(MppVencCtx *ctx, MppData *sink_data) {
    S32 ret = 0;
    ret = handle_venc_data(ctx->pAlBaseContext, sink_data);
...
S32 venc_ctx_process(MppVencCtx *ctx, MppData *sink_data, MppData *src_data) {
    S32 ret = 0;
    ret = process_venc_data(ctx->pAlBaseContext, sink_data, src_data);
    return ret;
}
...
S32 venc_ctx_get_output_stream(MppVencCtx *ctx, MppData *src_data) {
    S32 ret = 0;
    ret = get_venc_result_sync(ctx->pAlBaseContext, src_data);
    return ret;
}
...
S32 venc_ctx_request_output_stream(MppVencCtx *ctx, MppData *src_data) {
    S32 ret = 0;
    ret = get_venc_result(ctx->pAlBaseContext, src_data);
    return ret;
}
...
S32 venc_ctx_return_output_stream(MppVencCtx *ctx, MppData *src_data) {
    S32 ret = 0;
    ret = return_venc_result(ctx->pAlBaseContext, src_data);
    return ret;
}
...
S32 venc_ctx_flush(MppVencCtx *ctx) {
    ...
    debug("begin flush!");
    ret = enc_flush(ctx->pAlBaseContext);
...
S32 venc_ctx_destroy(MppVencCtx *ctx) {
    ...
    if (enc_destory)
        enc_destory(ctx->pAlBaseContext);
