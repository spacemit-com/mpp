...
/* Strip H.264/H.265 emulation prevention bytes: any 0x000003 sequence in the
 * NAL payload has the 0x03 removed to recover the raw RBSP. Returns the RBSP
 * length written to pu8Out (capacity u32OutCap). */
static U32 nal_to_rbsp(const U8 *pu8In, U32 u32InLen, U8 *pu8Out, U32 u32OutCap) {
    ...
    for (U32 i = 0; i < u32InLen; i++) {
        U8 b = pu8In[i];
        /* Drop the 0x03 in a 0x00 0x00 0x03 run (emulation_prevention_three_byte). */
...
S32 H264_ParseSps(const U8 *pu8Sps, U32 u32Len, U32 *pu32Width, U32 *pu32Height) {
    ...
    /* Strip emulation prevention bytes (0x000003) before bitstream parsing,
     * otherwise embedded 0x03 bytes corrupt the parsed field values. The NAL
     * header byte is preserved at index 0 and skipped by BitReader_Init. */
    U8 au8Rbsp[512];
    ...
    BitReader br;
    BitReader_Init(&br, au8Rbsp + 1, u32RbspLen - 1); /* Skip NAL header */

    /* profile_idc */
    U32 profile = BitReader_ReadBits(&br, 8);

    /* Default chroma_format is 1 (4:2:0) per spec when not present. */
    U32 chroma_format = 1;
    ...
    if (profile == 100 || profile == 110 || profile == 122 || profile == 244 || profile == 44 || profile == 83 ||
        ...
        chroma_format = BitReader_ReadUE(&br);
        if (chroma_format == 3) {
    ...
    /* Calculate dimensions with overflow/underflow safety.
     * Compute using 64-bit temporaries and validate crop offsets do not
     * exceed the un-cropped size to avoid unsigned underflow. Crop units
     * depend on chroma_format and whether frames are coded as frames/fields.
     * See H.264 spec for crop unit definitions. Return -1 on malformed SPS. */
    U64 width64 = (U64)pic_width_mbs * 16ULL;
    U64 height64 = (U64)(2 - frame_mbs_only) * (U64)pic_height_map * 16ULL;

    U32 subWidthC = (chroma_format == 1 || chroma_format == 2) ? 2U : 1U;
    U32 subHeightC = (chroma_format == 1) ? 2U : 1U;

    U64 cropUnitX = (U64)subWidthC;
    U64 cropUnitY = (U64)subHeightC * (U64)(2 - frame_mbs_only);

    U64 widthOffset = (U64)(crop_left + crop_right) * cropUnitX;
    U64 heightOffset = (U64)(crop_top + crop_bottom) * cropUnitY;

    if (widthOffset > width64 || heightOffset > height64) {
        return -1;
    }

    *pu32Width = (U32)(width64 - widthOffset);
    *pu32Height = (U32)(height64 - heightOffset);
...
S32 H264_AnnexBToAvcc(const U8 *pu8In, U32 u32InLen, U8 *pu8Out, U32 u32OutCap, U32 *pu32OutLen) {
    ...
    while (u32Pos < u32InLen) {
        ...
        /* Ensure the 4-byte length prefix plus NAL data fits the output. */
        if (u32OutPos + 4 + nalLen > u32OutCap)
            return -1;
...
S32 H264_AvccToAnnexB(const U8 *pu8In, U32 u32InLen, U8 *pu8Out, U32 u32OutCap, U32 *pu32OutLen, U8 u8NalLenSize) {
    ...
    while (u32Pos + u8NalLenSize <= u32InLen) {
        ...
        if (u32Pos + nalLen > u32InLen)
            return -1; /* declared NAL length exceeds input: truncated/corrupt */

        /* Ensure the 4-byte start code plus NAL data fits the output. */
        if (u32OutPos + 4 + nalLen > u32OutCap)
            return -1;
