...
S32 AAC_ParseAdts(const U8 *pu8Data, U32 u32Len, AdtsHeader *pstHeader) {
    if (!pu8Data || u32Len < ADTS_HEADER_SIZE || !pstHeader)
        return -1;
     ...
     * end of the buffer.
     */
    if (pstHeader->u16FrameLen < ADTS_HEADER_SIZE || pstHeader->u16FrameLen > u32Len) {
        return -1;
    }

    pstHeader->u32SampleRate = AAC_GetSampleRate(pstHeader->u8SampleRateIdx);

    /* sampling_frequency_index 13..15 are reserved/escape values and map to a
     * sample rate of 0 in the table. Returning success with u32SampleRate == 0
     * would let callers build audio parameters with a zero rate (division by
     * zero, bad timestamps), so reject these as malformed ADTS. */
    if (pstHeader->u32SampleRate == 0) {
        return -1;
    }
...
S32 AAC_CreateAdts(U8 *pu8Header, U8 u8Profile, U8 u8SampleRateIdx, U8 u8ChannelConfig, U16 u16DataLen) {
    ...
    /*
     * The ADTS profile field is only 2 bits wide and is encoded as
     * (audioObjectType - 1). Only AOT 1..4 (Main/LC/SSR/LTP) map to the
     * 4 representable values; anything else (notably 0, which would make
     * "(u8Profile - 1) << 6" wrap to 0xC0 and corrupt the header) is invalid.
     */
    if (u8Profile < 1 || u8Profile > 4) {
        return -1;
    }

    /* The 4-bit sampling_frequency_index (0..15; 15 is "explicit", unusable
     * here) and 3-bit channel_configuration must fit their header fields. */
    if (u8SampleRateIdx > 14 || u8ChannelConfig > 7) {
        return -1;
    }

    /*
     * The ADTS aac_frame_length field is only 13 bits wide, so the total
     * frame size (payload + 7-byte header) must not exceed 8191. A larger
     * value would be silently truncated when packed into the header,
     * producing a corrupt stream. Reject it instead.
     */
    if ((U32)u16DataLen + ADTS_HEADER_SIZE > ADTS_MAX_FRAME_LEN) {
        return -1;
    }

    U16 frameLen = u16DataLen + ADTS_HEADER_SIZE; /* Include header */
