...
S32 ByteBuffer_Reserve(ByteBuffer *pBuf, U32 u32Capacity) {
    ...
    while (newCap < u32Capacity) {
        /* Guard against unsigned overflow when doubling the capacity.
         * Once doubling would wrap past UINT32_MAX, fall back to the exact
         * requested capacity instead of looping forever on a wrapped value. */
...
S32 ByteBuffer_Append(ByteBuffer *pBuf, const U8 *pu8Data, U32 u32Len) {
    ...
    /* Reject sizes that would overflow the total length computation. */
    if (u32Len > 0xFFFFFFFFU - pBuf->u32Size)
        return -1;
