...
U32 BitReader_ReadUE(BitReader *pBr)
{
    ...
    while (!BitReader_IsEof(pBr) && BitReader_ReadBits(pBr, 1) == 0) {
        u32LeadingZeros++;
        /* A valid Exp-Golomb code in H.264/H.265 never exceeds 31 leading
         * zeros. Bail out on malformed/oversized input to avoid the undefined
         * behaviour of shifting a 32-bit value by >= 32 bits below. */
        if (u32LeadingZeros >= 32) {
            return 0;
        }
    }
    ...
    /* Use unsigned 1U: "1 << 31" on a signed int would overflow (UB). */
    return (1U << u32LeadingZeros) - 1U + u32Value;
}
