...
S32 vdec_ctx_init(MppVdecCtx *ctx) {
  ...
  dec_return_output_frame = (S32(*)(ALBaseContext * ctx, MppData * src_data))
      dlsym(handle, "al_dec_return_output_frame");
  dec_queue_output_buffer = (S32(*)(ALBaseContext * ctx, MppData * src_data))
      dlsym(handle, "al_dec_queue_output_buffer");
  dec_destory = (void (*)(ALBaseContext * ctx))
      dlsym(handle, "al_dec_destory");
  dec_flush = (S32(*)(ALBaseContext * ctx)) dlsym(handle, "al_dec_flush");
  dec_reset = (S32(*)(ALBaseContext * ctx)) dlsym(handle, "al_dec_reset");

  if (!dec_create || !dec_init || !dec_decode || !dec_return_output_frame ||
      !dec_destory) {
    error("required decoder symbols missing from plugin (create/init/decode/"
          "return/destroy)");
    module_destory(ctx->pModule);
    ctx->pModule = NULL;
    return MPP_INIT_FAILED;
...
S32 vdec_ctx_request_output_frame_2(MppVdecCtx *ctx, MppData **src_data,
                                    U32 u32TimeoutMs) {
  S32 ret = 0;
  ret = get_vdec_result_2(ctx->pNode.pAlBaseContext, src_data, u32TimeoutMs);
...
S32 vdec_ctx_queue_output_buffer(MppVdecCtx *ctx, MppData *src_data) {
  S32 ret = 0;
  ret = dec_queue_output_buffer(ctx->pNode.pAlBaseContext, src_data);
