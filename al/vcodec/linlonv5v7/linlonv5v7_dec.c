...
static S32 checkInputParameters(MppCodingType type, S32 profile,
  ...
  if (format != MPP_PIXEL_FORMAT_I420 && format != MPP_PIXEL_FORMAT_NV12 &&
      format != MPP_PIXEL_FORMAT_NV21) {
...
S32 al_dec_decode(ALBaseContext *ctx, MppData *sink_data) {
  ...
  if (unlikely(context->nInputQueuedNum <
               (U32)getBufNum(getInputPort(context->stCodec)))) {
    Buffer *buf =
...
RETURN al_dec_request_output_frame_2(ALBaseContext *ctx, MppData **src_data,
                                     U32 u32TimeoutMs) {
  if (!ctx || !src_data) {
...
RETURN al_dec_return_output_frame(ALBaseContext *ctx, MppData *src_data) {
  ...
  if (!buf) {
    ...
    if (getPortBufferType(getOutputPort(context->stCodec)) ==
        MPP_FRAME_BUFFERTYPE_DMABUF_EXTERNAL) {
      setExternalDmaBuf(buf, FRAME_GetFD(frame, 0),
                        (U8 *)FRAME_GetDataPointer(frame, 0), buf_idx);
    }
