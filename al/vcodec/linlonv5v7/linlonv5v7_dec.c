 ...
 * };
 */
static void setDecoderDSLRatio(ALLinlonv5v7DecContext *context, S32 hor, S32 ver) {
...
static S32 checkInputParameters(MppCodingType type, S32 profile, MppPixelFormat format) {
    if (type != CODING_H264 && type != CODING_H265 && type != CODING_MJPEG && type != CODING_VP8 &&
        type != CODING_VP9 && type != CODING_MPEG2 && type != CODING_MPEG4) {
...
RETURN al_dec_init(ALBaseContext *ctx, MppVdecPara *para) {
    ...
    ret = checkInputParameters(para->eCodingType, para->nProfile, para->eOutputPixelFormat);
    if (ret) {
    ...
    context->nInputFormatFourcc = get_linlonv5v7dec_codec_coding_type(context->eCodingType);
    ...
    } else if (para->eFrameBufferType == MPP_FRAME_BUFFERTYPE_DMABUF_EXTERNAL) {
        context->nOutputMemType = V4L2_MEMORY_DMABUF;
    ...
    context->nVideoFd = find_v4l2_decoder(context->sDevicePath, get_linlonv5v7dec_codec_coding_type(para->eCodingType));
    ...
    setDecoderDSLFrame(
        context, context->pVdecPara->nHorizonScaleDownFrameWidth, context->pVdecPara->nVerticalScaleDownFrameHeight);
...
S32 al_dec_decode(ALBaseContext *ctx, MppData *sink_data) {
    ...
    if ((!PACKET_GetLength(packet))) {
        debug("length of input packet is 0, EOS is coming, pts(%ld)", PACKET_GetPts(packet));
        context->bInputEos = MPP_TRUE;
        context->nEosPts = PACKET_GetPts(packet);
    }

    if (PACKET_GetEos(packet)) {
        debug("eos flag of input packet is set, EOS is coming(%ld)", PACKET_GetPts(packet));
        context->bInputEos = MPP_TRUE;
        context->nEosPts = PACKET_GetPts(packet);
    }
    ...
    if (unlikely(context->nInputQueuedNum < (U32)getBufNum(getInputPort(context->stCodec)))) {
        ...
        memcpy(getUserPtr(buf, 0), PACKET_GetDataPointer(packet), PACKET_GetLength(packet));
#if 0
    debug("input check:%x %x %x %x %x %x %x %x %x",
        ...
        if (MPP_OK == ret && p.revents & POLLOUT) {
            ret = handleInputBuffer(getInputPort(context->stCodec), context->bInputEos, sink_data);
            if (ret) {
...
RETURN al_dec_request_output_frame(ALBaseContext *ctx, MppData *src_data) {
    ...
    if (MPP_OK == ret && p.revents & POLLIN) {
        ret = handleOutputBuffer(getOutputPort(context->stCodec), MPP_FALSE, src_data);
        // debug("ok, a frame is handled, get it! ret = %d", ret);
        if (ret == MPP_RESOLUTION_CHANGED) {
            if (context->nRotation == 90 || context->nRotation == 270) {
                context->pVdecPara->nWidth = getBufHeight(getOutputPort(context->stCodec));
                context->pVdecPara->nHeight = getBufWidth(getOutputPort(context->stCodec));
            } else {
            ...
            for (S32 i = 0; i < context->pVdecPara->nOutputBufferNum; i++) {
            ...
            if (context->nEosPts > 0 && FRAME_GetPts(FRAME_GetFrame(src_data)) == context->nEosPts) {
...
RETURN al_dec_request_output_frame_2(ALBaseContext *ctx, MppData **src_data, U32 u32TimeoutMs) {
    if (!ctx || !src_data) {
    ...
    *src_data = NULL;
    ...
    /* Create MppFrame to receive the dequeued buffer */
    MppFrame *pFrame = FRAME_Create();
    if (!pFrame) {
        error("FRAME_Create failed");
        return MPP_MALLOC_FAILED;
    }
    MppData *data = FRAME_GetBaseData(pFrame);

    ret = handleOutputBuffer(getOutputPort(context->stCodec), MPP_FALSE, data);
    if (ret == MPP_RESOLUTION_CHANGED) {
        if (context->nRotation == 90 || context->nRotation == 270) {
            context->pVdecPara->nWidth = getBufHeight(getOutputPort(context->stCodec));
            context->pVdecPara->nHeight = getBufWidth(getOutputPort(context->stCodec));
        ...
        if (context->nEosPts > 0 && FRAME_GetPts(frame) == context->nEosPts) {
            error("it is a EOS frame eos pts:(%ld)", context->nEosPts);
            FRAME_Destory(pFrame);
            return MPP_CODER_EOS;
        }
    ...
    if (ret == MPP_OK) {
        MppFrame *frame = FRAME_GetFrame(data);
        context->pVdecPara->bIsBufferInDecoder[FRAME_GetID(frame)] = MPP_FALSE;
    }
...
RETURN al_dec_return_output_frame(ALBaseContext *ctx, MppData *src_data) {
    ...
    if (!buf) {
        ...
        if (getPortBufferType(getOutputPort(context->stCodec)) == MPP_FRAME_BUFFERTYPE_DMABUF_EXTERNAL) {
            setExternalDmaBuf(buf, FRAME_GetFD(frame, 0), (U8 *)FRAME_GetDataPointer(frame, 0), buf_idx);
        }
...
RETURN al_dec_queue_output_buffer(ALBaseContext *ctx, MppData *src_data) {
    if (!ctx) {
        error("input para ALBaseContext is NULL, please check!");
        return MPP_NULL_POINTER;
    }
