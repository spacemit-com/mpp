 ...
 */
static void setEncoderRateControl(ALLinlonv5v7EncContext *context, const char *rc,
...
S32 al_enc_send_input_frame(ALBaseContext *ctx, MppData *sink_data) {
  ...
  if (unlikely(context->nInputQueuedNum <
    ...
    if (context->nInputMemType == V4L2_MEMORY_USERPTR) {
      if (context->ePixelFormat == MPP_PIXEL_FORMAT_NV12 ||
          context->ePixelFormat == MPP_PIXEL_FORMAT_NV21) {
