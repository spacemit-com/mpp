...
static VOID K1_VI_FillDwtPlanes(
    IMAGE_BUFFER_S *pstImageBuffer, U32 u32BaseWidth, U32 u32BaseHeight, int iFd, void *pBaseVir, U32 *pu32Offset
) {
    U32 u32Level = 0;
...
S32 K1_VI_ImportExternalBufPool(
    ...
    if (pstChnCtx == NULL || paulBufferId == NULL || pastFrameInfo == NULL || pastImageBuffer == NULL ||
        u32BufCnt == 0 || u32BufCnt > K1_VI_MAX_BUF_CNT)
...
K1_VI_BUF_NODE_S *K1_VI_GetIdleBufNode(K1_VI_CHN_CTX_S *pstChnCtx) {
    ...
    for (i = 0; i < pstChnCtx->u32BufCnt; i++) {
        if (pstChnCtx->astBufNode[i].bValid == MPP_TRUE && pstChnCtx->astBufNode[i].enState == K1_VI_BUF_STATE_IDLE) {
            return &pstChnCtx->astBufNode[i];
        }
    }
...
S32 K1_VI_QueueBufNode(K1_VI_CHN_CTX_S *pstChnCtx, K1_VI_BUF_NODE_S *pstBufNode) {
    ...
    // pstBufNode->ulBufferId);
    //  info("pstChnCtx->ulVbPool: %lu\n", pstChnCtx->ulVbPool);
    //  info("pstChnCtx->u32AsrChn: %u\n", pstChnCtx->u32AsrChn);

    // info("test_buffer_prepare: buffer fd %d, width %d, height %d, stride %d\n",
    // pstBufNode->stImageBuffer.m.fd,pstBufNode->stImageBuffer.planes[0].width,
    // pstBufNode->stImageBuffer.planes[0].height, pstBufNode->stImageBuffer.planes[0].stride);
    // info("test_buffer_prepare: type %d, format %d\n", pstBufNode->stImageBuffer.type,
    // pstBufNode->stImageBuffer.format); info("test_buffer_prepare: buffer plane0 size %d plane1 size %d\n",
    ...
    // pstBufNode->stImageBuffer.size.height, pstBufNode->stImageBuffer.format);
...
VOID K1_VI_FillImageBufferFromVideoFrame(const VideoFrameInfo *fi, IMAGE_BUFFER_S *ib) {
    ...
    ib->format = (int)K1_VI_ToAsrImagePixelFormat(fi->stViFrameInfo.stCommFrameInfo.ePixelFormat);
    ...
    if (fi->stViFrameInfo.stCommFrameInfo.ePixelFormat == MPP_PIXEL_FORMAT_NV12 ||
        ...
        K1_VI_FillDwtPlanes(ib, ib->size.width, ib->size.height, iFd, pBaseVir, &u32Offset);
    }
