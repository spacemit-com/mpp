...
MppPacket *PACKET_Create() {
    ...
    mpp_env_get_u32("MPP_PRINT_UNFREE_PACKET", &(packet->bEnableUnfreePacketDebug), 0);

    if (packet->bEnableUnfreePacketDebug) {
        num_of_unfree_packet++;
        info("++++++++++ debug packet memory: num of unfree packet: %d", num_of_unfree_packet);
    }
...
RETURN PACKET_Alloc(MppPacket *packet, S32 size) {
    ...
    if (!packet->pData) {
        error("can not malloc MppPacket->pData, please check! (%s)", strerror(errno));
    ...
    if (packet->bEnableUnfreePacketDebug) {
        ...
        info("++++++++++ debug packet memory: num of unfree packet data: %d", num_of_unfree_packet_data);
    }
