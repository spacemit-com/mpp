...
void RingBufferPrint(MppRingBuffer *rBuf, BOOL hex) {
    ...
    for (U32 i = 0; i < cSize; i++) {
        ...
        if (hex)
            snprintf(str + i * 2, 4, "%02X|", c);  // NOLINT(runtime/printf)
        else
            snprintf(str + i * 2, 3, "%c|", c);  // NOLINT(runtime/printf)
    }
...
void TestRingBuffer(void) {
    ...
    for (j = 0; j < 10; j++) {
        for (i = 0; i < 3; i++) {
            // push
            debug("=====================================");
            len = random_number = rand_r(&rand_seed) % 3 + 1;
    ...
    for (j = 0; j < 30; j++) {
        for (i = 0; i < 1; i++) {
            // push
            debug("=====================================");
            len = random_number = rand_r(&rand_seed) % 2 + 1;
            debug("++push=> %d bytes chars:(%c - %c)", len, *(a + offset),
                        *(a + offset + len - 1));
            RingBufferPush(cb, a + offset, len);
            offset += len;
    ...
    for (j = 0; j < 30; j++) {
        for (i = 0; i < 1; i++) {
            // push
            debug("=====================================");
            len = random_number = rand_r(&rand_seed) % 4 + 1;
            debug("++push=> %d bytes chars:(%c - %c)", len, *(a + offset),
                        *(a + offset + len - 1));
            RingBufferPush(cb, a + offset, len);
            offset += len;
    ...
    for (j = 0; j < 30; j++) {
        for (i = 0; i < 1; i++) {
            // push
            debug("=====================================");
            len = random_number = rand_r(&rand_seed) % 7 + 1;
            debug("++push=> %d bytes chars:(%c - %c)", len, *(a + offset),
                        *(a + offset + len - 1));
            RingBufferPush(cb, a + offset, len);
            offset += len;
