...
MppFrame *FRAME_Create() {
    ...
    mpp_env_get_u32("MPP_PRINT_UNFREE_FRAME", &(frame->bEnableUnfreeFrameDebug), 0);

    if (frame->bEnableUnfreeFrameDebug) {
        num_of_unfree_frame++;
        info("++++++++++ debug frame memory: num of unfree frame: %d", num_of_unfree_frame);
    }
    ...
    if (MPP_FRAME_BUFFERTYPE_NORMAL_INTERNAL == frame->eBufferType) {
        ...
        for (i = 0; i < frame->nDataUsedNum; i++) {
            if (i == 0) {
                ...
                if (!frame->pData0) {
                    error("can not malloc MppFrame->pData0, please check! (%s)", strerror(errno));
                ...
                if (!frame->pData1) {
                    error("can not malloc MppFrame->pData1, please check! (%s)", strerror(errno));
                    free(frame->pData0);
                    return MPP_MALLOC_FAILED;
                }
                ...
                if (!frame->pData2) {
                    error("can not malloc MppFrame->pData2, please check! (%s)", strerror(errno));
                    free(frame->pData0);
                    free(frame->pData1);
                    return MPP_MALLOC_FAILED;
                }
...
RETURN FRAME_Free(MppFrame *frame) {
    ...
    if (MPP_FRAME_BUFFERTYPE_NORMAL_INTERNAL == frame->eBufferType) {
        ...
        if (frame->bEnableUnfreeFrameDebug) {
            num_of_unfree_data--;
            info("debug frame memory: num of unfree frame data: %d", num_of_unfree_data);
        }
    } else if (
        MPP_FRAME_BUFFERTYPE_NORMAL_EXTERNAL == frame->eBufferType ||
...
void *FRAME_GetDataPointer(MppFrame *frame, S32 data_num) {
    ...
    if (data_num < 0 || data_num >= frame->nDataUsedNum) {
        error("data_num is not valid %d %d, please check!", data_num, frame->nDataUsedNum);
        return NULL;
    }
...
RETURN FRAME_SetDataPointer(MppFrame *frame, S32 data_num, U8 *data) {
    ...
    if (data_num < 0 || data_num >= frame->nDataUsedNum) {
        error("input para data_num is not valid %d %d, please check!", data_num, frame->nDataUsedNum);
        return MPP_CHECK_FAILED;
    }
