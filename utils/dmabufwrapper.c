...
DmaBufWrapper *createDmaBufWrapper(DMAHEAP heap) {
    ...
    if (wrapper_tmp->nDmaHeapFd < 0) {
        error("can not open (%s), fd < 0!!! (%s)", dma_heap_path[heap], strerror(errno));
        free(wrapper_tmp);
        return NULL;
    }

    mpp_env_get_u32("MPP_PRINT_UNFREE_DMABUF", &(wrapper_tmp->bEnableUnfreeDmaBufDebug), 0);
...
void *mmapDmaBuf(DmaBufWrapper *context) {
    ...
    context->sDmaBuf.pVaddr =
        (void *)mmap(0, context->sDmaBuf.nSize, PROT_READ | PROT_WRITE, MAP_SHARED, context->sDmaBuf.nFd, 0);
