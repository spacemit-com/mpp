...
#define LOG_INFO(fmt, ...)                            \
    do {                                              \
        if (g_enLogLevel >= VI_DEMO_LOG_LEVEL_NORMAL) \
            printf("[INFO] " fmt, ##__VA_ARGS__);     \
    } while (0)

#define LOG_VERBOSE(fmt, ...)                          \
    do {                                               \
        if (g_enLogLevel >= VI_DEMO_LOG_LEVEL_VERBOSE) \
            printf("[VERBOSE] " fmt, ##__VA_ARGS__);   \
    } while (0)

#define LOG_DEBUG(fmt, ...)                          \
    do {                                             \
        if (g_enLogLevel >= VI_DEMO_LOG_LEVEL_DEBUG) \
...
static VOID VI_DemoFpsStatUpdate(VI_DEMO_FPS_STAT_S *pstStat) {
    ...
    if (dWindowMs >= 1000.0) {
        ...
        LOG_VERBOSE(
            "[%s][FPS] frames=%u elapsed=%.2f ms fps=%.2f\n",
...
static VOID VI_DemoFpsStatPrintFinal(const VI_DEMO_FPS_STAT_S *pstStat) {
    ...
    LOG_INFO(
        "[%s][FPS][FINAL] frames=%u elapsed=%.2f ms avg_fps=%.2f\n",
        pstStat->tag != NULL ? pstStat->tag : "unknown",
        pstStat->u32FrameCount,
        dTotalMs,
        dFps);
...
static S32 VI_DemoParseFrameRateCtrlToken(const char *pszToken, BOOL *pbEnabled, ViFrameRateCtrlS *pstCtrl) {
    U32 u32Input = 0;
    U32 u32Output = 0;
...
static S32 VI_DemoApplyFrameRateArg(VI_DEMO_CONFIG_S *pstCfg, const char *pszArg) {
    ...
    if (uKeyLen == 5 && strncmp(pszArg, "virt1", 5) == 0)
        return VI_DemoParseFrameRateCtrlToken(
            pszEq + 1, &pstCfg->bEnableVirt1FrameRateCtrl, &pstCfg->stVirt1FrameRateCtrl);
    if (uKeyLen == 5 && strncmp(pszArg, "virt2", 5) == 0)
        return VI_DemoParseFrameRateCtrlToken(
            pszEq + 1, &pstCfg->bEnableVirt2FrameRateCtrl, &pstCfg->stVirt2FrameRateCtrl);
    if (uKeyLen == 5 && strncmp(pszArg, "virt3", 5) == 0)
        return VI_DemoParseFrameRateCtrlToken(
            pszEq + 1, &pstCfg->bEnableVirt3FrameRateCtrl, &pstCfg->stVirt3FrameRateCtrl);
    if (uKeyLen == 5 && strncmp(pszArg, "virt4", 5) == 0)
        return VI_DemoParseFrameRateCtrlToken(
            pszEq + 1, &pstCfg->bEnableVirt4FrameRateCtrl, &pstCfg->stVirt4FrameRateCtrl);
    if (uKeyLen == 5 && strncmp(pszArg, "virt5", 5) == 0)
        return VI_DemoParseFrameRateCtrlToken(
            pszEq + 1, &pstCfg->bEnableVirt5FrameRateCtrl, &pstCfg->stVirt5FrameRateCtrl);
...
static void VI_DemoDumpFrameInfo(const char *tag, VI_DEV ViDev, VI_CHN ViChn, const VideoFrameInfo *pstFrame) {
    ...
    LOG_DEBUG(
        "[%s] dev=%d chn=%d idx=%u buf=%lu pool=%lu pts=%" PRIu64 "size=%ux%u fmt=%d total=%u planes=%u valid0=%u\n",
        tag,
        ViDev,
        ViChn,
        pstFrame->u32Idx,
...
static S32 VI_DemoGetAndReleaseFrame(
    VI_DEV ViDev,
    VI_CHN ViChn,
    S32 s32TimeoutMs,
    const char *tag,
...
static S32 VI_DemoRunRawDump(
    VI_DEV ViDev, VI_CHN ViChn, S32 s32TimeoutMs, const VI_DEMO_CONFIG_S *pstCfg, void *pstDumpState
) {
    ...
    // sleep 200ms to wait raw frame ready, or directly call VI_GetRawDumpFrame may get -4
    usleep(200 * 1000);
...
static S32 VI_DemoSetup(const VI_DEMO_CONFIG_S *pstCfg) {
    ...
    LOG_INFO(
        "phy attr: %ux%u mirror=%d flip=%d rotate=%d crop=%d[%u,%u,%u,%u] align=%d\n",
    ...
    if (pstCfg->bEnableVirt1 == MPP_TRUE) {
        ...
        LOG_INFO(
            "virt1 attr: %ux%u mirror=%d flip=%d rotate=%d crop=%d[%u,%u,%u,%u] align=%d\n",
    ...
    if (pstCfg->bEnableVirt2 == MPP_TRUE) {
        ...
        LOG_INFO(
            "virt2 attr: %ux%u mirror=%d flip=%d rotate=%d crop=%d[%u,%u,%u,%u] align=%d\n",
    ...
    if (pstCfg->bEnableVirt3 == MPP_TRUE) {
        ...
        LOG_INFO(
            "virt3 attr: %ux%u mirror=%d flip=%d rotate=%d crop=%d[%u,%u,%u,%u] align=%d\n",
    ...
    if (pstCfg->bEnableVirt4 == MPP_TRUE) {
        ...
        LOG_INFO(
            "virt4 attr: %ux%u mirror=%d flip=%d rotate=%d crop=%d[%u,%u,%u,%u] align=%d\n",
    ...
    if (pstCfg->bEnableVirt5 == MPP_TRUE) {
        ...
        LOG_INFO(
            "virt5 attr: %ux%u mirror=%d flip=%d rotate=%d crop=%d[%u,%u,%u,%u] align=%d\n",
...
static S32 VI_DemoRunBasic(const VI_DEMO_CONFIG_S *pstCfg) {
    ...
    for (i = 0; i < pstCfg->u32FrameCount; i++) {
        s32Ret = VI_DemoGetAndReleaseFrame(
            pstCfg->ViDev,
            pstCfg->ViPhyChn,
            pstCfg->s32TimeoutMs,
            "phy0",
    ...
    if (pstCfg->bEnablePhyFrameRateCtrl == MPP_TRUE) {
        LOG_INFO(
            "[verify][phy0] expected fps ~= source_fps * %u / %u\n",
...
static S32 VI_DemoRunRawDumpOnly(const VI_DEMO_CONFIG_S *pstCfg) {
    ...
    for (i = 0; i < 3; i++) {
        s32Ret = VI_DemoGetAndReleaseFrame(
            pstCfg->ViDev, pstCfg->ViPhyChn, pstCfg->s32TimeoutMs, "phy0-before-raw", pstCfg, NULL, MPP_FALSE);
        if (s32Ret != 0)
            return s32Ret;
    }

    for (i = 0; i < pstCfg->u32FrameCount; i++) {
        s32Ret = VI_DemoRunRawDump(pstCfg->ViDev, pstCfg->ViPhyChn, pstCfg->s32TimeoutMs, pstCfg, NULL);
        if (s32Ret != 0)
            return s32Ret;
...
int main(int argc, char **argv) {
    ...
    LOG_INFO(
        "VI bringup demo mode=%s frame_count=%u log_level=%d\n",
        VI_DemoModeName(enMode),
        stCfg.u32FrameCount,
        g_enLogLevel);
