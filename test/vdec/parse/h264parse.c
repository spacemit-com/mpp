 ...
 * @LastEditTime: 2024-04-29 19:41:41
 * @Description:
 */
...
S32 PARSE_H264_Parse(
    ...
    while (1) {
        ...
        if (((0x00 == *(tmp_mem)) && (0x00 == *(tmp_mem + 1)) && (0x00 == *(tmp_mem + 2)) &&
                (0x01 == *(tmp_mem + 3))) ||
            ((0x00 == *(tmp_mem)) && (0x00 == *(tmp_mem + 1)) && (0x01 == *(tmp_mem + 2))))

        {
            ...
            // SEI&SPS&PPS
            if (0x06 == tmp_0 || 0x07 == tmp_0 || 0x08 == tmp_0) {
                hd_start_code++;
                if (0 == num_start_code && 1 == hd_start_code) {
                    start_pos = rbsp_base;
                }
                if (1 == num_start_code)
                    num_start_code++;
            ...
            if (0x01 == tmp_0 || 0x05 == tmp_0)  // non-IDR picture, IDR picture
            {
                U8 umpt;
                U8 *temp;
                // S32 frame_type;
                 ...
                 */
                tmp_1 = *(tmp_mem + 4) & 0x80;
                if (0x80 == tmp_1)  // first slice of frame
                {
                    /*Here maybe the first slice, or the first slice of next frame,
                     * it is determined by num_start_code and hd_start_code.
                     */
                    num_start_code++;
                    if (1 == num_start_code && 0 == hd_start_code)
                        start_pos = rbsp_base;
                }

                if (1 == num_start_code)  // find the first I/B/P header（should not return）
                {
    ...
    if (2 == num_start_code) {
        ...
        return 0;
    } else if (1 == num_start_code) {
        error("stream_size = %d", stream_size);
        *frame_size = stream_size;
        memcpy(frame_start_addr, start_pos, stream_size);
        return 0;
    } else {
        return -1;
    }
