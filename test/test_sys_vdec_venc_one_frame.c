 ...
 * @LastEditTime: 2023-11-09 14:34:32
 * @Description:
 */
...
int main(int argc, char **argv) {
  ...
  src_packet = PACKET_Create();
  PACKET_Alloc(src_packet, 120000);

  SYS_Getresult(ctx, PACKET_GetBaseData(src_packet));

  fwrite(PACKET_GetDataPointer(src_packet), PACKET_GetLength(src_packet), 1,
         fp1);
  fflush(fp1);
